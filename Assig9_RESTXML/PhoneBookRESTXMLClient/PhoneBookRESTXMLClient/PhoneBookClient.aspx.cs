            ...
            if (IsPostBack)
            {
                   ...
                   (!string.IsNullOrEmpty(phoneTextBox.Text)))
                {
                    HttpResponseMessage response =
                       await client.GetAsync(new Uri("http://localhost:52163/PhoneBookRESTXMLService.svc/" + ADD_ENTRY + "/"
                       + lastTextBox.Text + "/" + firstTextBox.Text + "/" + phoneTextBox.Text));
                    ...
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                        ...
                        resultsTextBox.Text = "AddEntry failed with HTTP code " + response.StatusCode+response.Content;
                } // end if
                else if (findLastTextBox.Text != string.Empty) // send request to PhoneBookRESTXMLService if field is filled - 
                {
                    String result = await client.GetStringAsync(new Uri("http://localhost:52163/PhoneBookRESTXMLService.svc/" + GET_ENTRIES + "/" + findLastTextBox.Text));

                    XDocument xmlResponse = XDocument.Parse(result); // parse the returned XML string 
                    clearFields();
                    //resultsTextBox.Text += xmlResponse.Element(xmlNamespace + "ArrayOfPhoneBook").Value;
                    // if there are no phone book entries in response
                     if (xmlResponse.Element(xmlNamespace + "ArrayOfPhoneBook").Value == string.Empty)
                      {
                      ...
                      else
                      {
                         ...
                         foreach (XElement element in xmlResponse.Element(xmlNamespace + "ArrayOfPhoneBook").Descendants(xmlNamespace + "PhoneBook"))
                         {
                            resultsTextBox.Text += '\n' + 
                               string.Format("{0}, {1}, {2}",
                                  element.Element(xmlNamespace + "FirstName").Value.Trim(),
